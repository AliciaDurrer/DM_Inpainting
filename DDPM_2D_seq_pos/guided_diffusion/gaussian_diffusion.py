...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        assert t.shape == (B,)
        model_output = model(
            x=x, slice_nrs=current_sli_nr, ts=self._scale_timesteps(t), **model_kwargs
        )
        x = x[
            :, -1:, ...
        ...
        t_list = [0, 1, 2, 3, 10, 100, 999, 1000]
    ...
    def training_losses_segmentation(
        self, model, classifier, x_start, t, slices_nrs, model_kwargs=None, noise=None
    ):
        """
        ...
        if self.loss_type == LossType.MSE or self.loss_type == LossType.RESCALED_MSE:
            model_output = model(
                x_t, self._scale_timesteps(t), slices_nrs, **model_kwargs
            )

            # logger.log('model output', model_output.shape)
