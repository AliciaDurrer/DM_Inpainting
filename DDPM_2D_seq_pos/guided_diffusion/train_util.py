...
class TrainLoop:
                ...
                for b in range(0, nr_batches):
                    ...
                    if len(slicedict) > b * batch_size_vol + batch_size_vol:
                        for s in slicedict[
                            ...
                            out_batch_0.append(batch[:, 0, ..., s].clone().detach())
                            out_batch_1.append(batch[:, 1, ..., s].clone().detach())
                            out_prev.append(cond[..., s - 1].clone().detach())
                            out_cond.append(cond[..., s].clone().detach())
                            slinfo.append(s)

                        out_batch_0 = th.stack(out_batch_0)
                        out_batch_1 = th.stack(out_batch_1)
                        out_prev = th.stack(out_prev)
                        out_cond = th.stack(out_cond)

                        out_batch_0 = out_batch_0.squeeze(4)
                        out_batch_1 = out_batch_1.squeeze(4)
                        out_prev = out_prev.squeeze(1).squeeze(4)
                        out_cond = out_cond.squeeze(1).squeeze(4)

                        out_batch = (
                            th.stack((out_batch_0, out_batch_1, out_prev), dim=1)
                        ).squeeze(2)

                        p_s = path[0].split("/")[3]

                        self.run_step(out_batch, out_cond, th.tensor(slinfo))
    ...
    def run_step(self, batch, cond, slices_nrs):
        batch = th.cat((batch, cond), dim=1)

        cond = {}
        sample = self.forward_backward(batch, cond, slices_nrs)
    ...
    def forward_backward(self, batch, cond, slices_nrs):
