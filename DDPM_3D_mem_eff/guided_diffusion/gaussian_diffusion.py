...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        ...
        C = 1
        assert t.shape == (B,)
        # print('x shape', x.shape, B, C, model_kwargs)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        """
        # print('GAUSSIAN DIFFUSION TRAINING LOSSES SEGMENTATION')
        if model_kwargs is None:
            model_kwargs = {}
        if noise is None:
        ...
        res_t = self.q_sample(
            goal, t, noise=noise
        )  # add noise to the BL channel, FU stays "clean" --> during q, noise is only added to the ground truth!
