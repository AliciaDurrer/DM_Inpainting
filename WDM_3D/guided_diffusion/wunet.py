...
class Upsample(nn.Module):
    ...
    def __init__(
        ...
        if use_conv:
            self.conv = conv_nd(
                dims, self.channels * 7, self.out_channels * 7, 3, padding=1, groups=7
            )
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        if self.use_freq:
            ...
            x = (LLL / 3.0, (LLH, LHL, LHH, HLL, HLH, HHL, HHH))
        else:
            x = self.op(x)
        return x
...
class WaveletDownsample(nn.Module):
    ...
    def forward(self, x):
        ...
        x = th.cat((LLL, LLH, LHL, LHH, HLL, HLH, HHL, HHH), dim=1) / 3.0
        return self.conv(x)
...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        if up:
            self.h_upd = Upsample(
                channels, False, dims, resample_2d=resample_2d, use_freq=self.use_freq
            )
            self.x_upd = Upsample(
                channels, False, dims, resample_2d=resample_2d, use_freq=self.use_freq
            )
...
class WavUNetModel(nn.Module):
    ...
    def __init__(
        ...
        # INPUT block #
        ###############
        print("[InConv] In ch: {}, Base ch: {}".format(in_channels, model_channels))
        print(" ")
        ...
        for level, mult in enumerate(channel_mult):
            print("[LEVEL] Level: {}, Mult: {}, In ch: {}".format(level, mult, ch))
            for _ in range(num_res_blocks):  # Adding Residual blocks
                print(
                    "[ResBlock] In ch: {} Out ch: {}".format(ch, mult * model_channels)
                )
                layers = [
                    ResBlock(
                        channels=ch,
                ...
                ch = (
                    mult * model_channels
                )  # New input channels = channel_mult * base_channels
            ...
            print("[ResBlock + Down] In ch: {} Out ch: {}".format(ch, ch))
            print(" ")
            out_ch = ch
            layers = []
            layers.append(
            ...
            if self.progressive_input == "residual":
                layers.append(
                    WaveletDownsample(in_ch=input_pyramid_channels, out_ch=out_ch)
                )
        ...
        #########################
        print("[ResBlock] In ch: {} Out ch: {}".format(ch, ch))
        print("[ResBlock] In ch: {} Out ch: {}".format(ch, ch))
        print(" ")
        ...
        for level, mult in list(enumerate(channel_mult))[::-1]:
            ...
            for i in range(num_res_blocks + 1):  # Adding Residual blocks
                if not i == num_res_blocks:
                    ...
                    print("[ResBlock] In ch: {} Out ch: {}".format(ch, mid_ch))
                    layers = [
                        ResBlock(
                            ch,
                            time_embed_dim,
                    ...
                    out_ch = ch
                    print("[ResBlock + Up] In ch: {} Out ch: {}".format(mid_ch, out_ch))
                    print(" ")
                    layers.append(
                        ResBlock(
                            mid_ch,
        ...
        for i in range(num_res_blocks):
            print("[OutRes] In ch: {}, Out ch: {}".format(ch, ch))
            layers = [
                ResBlock(
                    ch,
                    time_embed_dim,
        ...
        # OUTPUT block #
        ################
        print("[OutConv] In ch: {}, Out ch: {}".format(model_channels, out_channels))
    ...
    def forward(self, x, timesteps):
        ...
        )  # Gen sinusoidal timestep embedding
        h = x
        self.hs_shapes = []
        ...
        for module in self.input_blocks:
            if not isinstance(module[0], WaveletDownsample):
                ...
                if isinstance(
                    h, tuple
                ):  # Check for skip features (tuple of high frequency subbands) and store in hs
                    h, skip = h
                hs.append(skip)
        ...
        for module in self.output_blocks:
            ...
            # Use frequency aware skip connections
                ...
                self.use_freq
            ):  # You usually want to use the frequency aware upsampling
